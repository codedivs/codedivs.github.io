...
const ADSENSE_SLOT_INFEED = "5533749814";            //"In-feed ad" 
// ==========================================================================

// ------------------------------------------------------------------
// Render games + inject native ad every 12 games
// ------------------------------------------------------------------
function renderGames(gamesToShow) {
  ...
  let realGameCounter = 0;

  gamesToShow.forEach(game => {
    // === 1. Insert REAL game card ===
    ...
    // === 2. Every 12 real games → insert NATIVE ADSENSE CARD ===
    if (realGameCounter % 12 === 0) {
      const adCard = document.createElement("section");
      adCard.className = "game-card ad-card"; // same styling as real cards
        ...
        <article class="game-card-inner">
          <div style="position:relative;">
            <img src="/assets/images/ad-placeholder.jpg" 
                 alt="Sponsored" 
                 class="game-icon" 
                 style="opacity:0.75; filter: brightness(0.8);">
            <div style="position:absolute; top:8px; right:8px; background:#4CAF50; color:#fff; 
                        font-size:11px; padding:3px 7px; border-radius:4px; font-weight:bold;">
              Sponsored
            </div>
          </div>
          <h2 class="game-title">Recommended Quiz</h2>
          <div class="game-footer" style="justify-content:center; background:#111;">
...
// ------------------------------------------------------------------
// FILTER BY CATEGORY – with shuffle preserved
// ------------------------------------------------------------------
function filterByCategory(selectedCat) {
  ...
  // Keep random order even after filtering
...
// ------------------------------------------------------------------
// SEARCH (unchanged, just minor cleanup)
// ------------------------------------------------------------------
function performSearch() {
  ...
  if (!query) {
    renderGames([...allGames].sort(() => 0.5 - Math.random()));
    return;
  }
...
fetch('games_in_library.json')
  ...
  .then(games => {
    ...
    // Initial load: fully shuffled + ads injected
    renderGames([...allGames].sort(() => 0.5 - Math.random()));
    createCategoryChips();
